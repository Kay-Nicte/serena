-- Restore get_daily_candidates after is_verified = TRUE filter was removed.
-- Uses only columns confirmed to exist in production (min_age, max_age, orientations,
-- looking_for, max_distance). Advanced filter columns (smoking_include_unspecified etc.)
-- are not yet applied to production, so they are excluded here.
